...
module.exports = class Shortfilm {
  ...
  static findEnabledScifi() {
    return db.execute('SELECT * FROM shortfilms WHERE enable_film = 1 AND gen_id = 1');
  }

  static findEnabledFantasy() {
    return db.execute('SELECT * FROM shortfilms WHERE enable_film = 1 AND gen_id = 2');
  }

  static findEnabledHorror() {
    return db.execute('SELECT * FROM shortfilms WHERE enable_film = 1 AND gen_id = 3');
  }

  static findEnabledAnimation() {
    return db.execute('SELECT * FROM shortfilms WHERE enable_film = 1 AND gen_id = 4');
  }
  ...
  static searchFilm(cont) {
    return db.execute(`SELECT * FROM shortfilms WHERE sinopsis like '%${cont}%' or title like '%${cont}%'`);
  }
